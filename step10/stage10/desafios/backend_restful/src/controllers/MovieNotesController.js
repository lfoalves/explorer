...
class MovieNotesController {
  async create(request, response) {
    ...
    if (!title) throw new AppError('Título para a nota é necessário.')

    const descriptionData = description ? description : null
    const ratingData = rating ? Number(rating) : null
  ...
  async delete(request, response) {
    ...
    await knex.connection('movie_notes').where({ id: note_id }).delete();
